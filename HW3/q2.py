...
def log_odds_ratio(seq, k, pos_kmer2idx, pos_trans_probs, neg_kmer2idx, neg_trans_probs):
    ...
    for i in range(len(seq) - k):
        ...
        score = score + np.log(prob_local_pos)-np.log(prob_local_neg)
...
def main(k):
    ...
    on the testing set, and generate predictions of q2_validation_data.csv that you will upload to
    ...
    Note: You do not have the ground truth labels for q2_validation_data.csv.
    """
    ...
    # loading predictions to q2_predictions.npy
    print("Fetching autograder validation data...")
    test_data = pd.read_csv("q2_validation_data.csv", sep=' ', header=None)
    ...
    print("Loading predictions to q2_predictions.npy...\n")
    np.save("q2_predictions_k={}".format(k), pred)

    """----- Instructor Version ----- (DO NOT INCLUDE IN THE HW)"""
    # test_label = np.load("q2_val_labels.npy")
...
if __name__ == '__main__':
    ...
    print("Don't forget to submit all the q2_predictions_k=*.npy along with your q1.py and q2.py!")
